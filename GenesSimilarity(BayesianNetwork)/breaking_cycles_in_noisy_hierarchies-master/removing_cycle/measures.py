...
def report_performance(gt_file, predicted_edges, note):
    ...
    if gt_file != None:
        ...
        print("%s 	%0.4f 	%0.4f 	%0.4f 	%d" %
              (note, accu, recall, F1_score, len(predicted_edges)))
        #print("method: %s, precision: %0.4f, recall: %0.4f, f1: %0.4f" % (note,accu,recall,F1_score))
    else:
        print("method: %s, # edges to be removed: %d" %
              (note, len(predicted_edges)))
    # print("**********************")
...
if __name__ == "__main__":
    ...
    parser.add_argument("-t", "--ground_truth", default=" ",
                        help="ground truth edges")
